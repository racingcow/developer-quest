...
.defines(function() {
	...
	ig.EntityPlayer = ig.global.EntityPlayer = ig.Player.extend({
		...
		name: 'Player',	
        facing: devquest.utilsvect.DIRECTION.DOWN, //Updated to reflect the current direction the player is facing
		...
		initProperties: function() {
			...
			this.whipAbility = new ig.AbilityWhip(this);
		...
		update: function() {		
			...
			if(ig.input.state('up') || ig.input.pressed('w')){
                ...
                this.facing = devquest.utilsvect.DIRECTION.UP;
			}
			else if(ig.input.state('down') || ig.input.pressed('s')){
				this.vel.y =  _c.CHARACTER.MAX_VEL_GROUNDED_Y;
				this.facing = devquest.utilsvect.DIRECTION.DOWN;
			}
			else {
				this.vel.y = 0;
			}
			...
			if(ig.input.state('left') || ig.input.pressed('a')){
				...
				this.facing = devquest.utilsvect.DIRECTION.LEFT;		
			}
			else if(ig.input.state('right') || ig.input.pressed('d')){
				this.vel.x = _c.CHARACTER.MAX_VEL_GROUNDED_X;
				this.facing = devquest.utilsvect.DIRECTION.RIGHT;
			}
			else {
				this.vel.x = 0;
			}
