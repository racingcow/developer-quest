...
ig.module(
	...
	.defines(function () {
		...
		devquest.ui.UIItemGrid = ig.global.UIItemGrid = UIElement.extend({
			...
			animSheetScroll: new ig.AnimationSheet(_c.PATH_TO_MEDIA + 'ui-item-grid-scroll.png', _cfg.SCROLL_SIZE_X, _cfg.SCROLL_SIZE_Y),
			animSettingsScroll: {
				track: {
					frameTime: 1,
					sequence: [0]
			...
			size: {
				x: _cfg.SPACING_X + _cfg.COLS_PER_PAGE * (_cfg.TILE_SIZE_X + _cfg.SPACING_X),
			...
			resetExtras: function() {
				...
				this.createAnim('track', {
					animSheet: this.animSheetScroll,
					animSettings: this.animSettingsScroll
				});

				// this.createAnim('scrollArrow', {
				// 	animSheet: this.animSheetScrollArrow,
				// 	animSettings: this.animSettingsScrollArrow
				// });
