	...
	.defines(function () {
		...
		devquest.ui.UIMenu = ig.global.UIMenu = ig.UIElement.extend({
			...
			initProperties: function() {
				...
				this.screenText = ig.game.spawnEntity(ig.UIText, 0, 0, {
					font: ig.Font.FONTS.MENU,
					text: 'screen',
					zIndex: _zIndex,
					linkedTo: this,
					linkAlignInside: true,
			...
			switchScreen: function(offset) {
				...
				this.screenText.text = this.screen.name;
				//this.screenText.refresh();
			...
			cleanupItem: function(item) {
				...
				//console.log('cleanupItem ' + item.name);
