	...
	function start() {
		ig.MainGame = ig.GameExtended.extend({
			...
			unpause: function ( unlock ) {
				this.unbindTriggers(); //prevent orphaned menu objects due to keypress machine-gunnery
				if (this.menu) {
					this.menu.fadeToDeath( { onComplete: this.bindTriggers } );
					this.menu = undefined;
				}
				this.parent();
			},
			...
			bindTriggers: function() {
				ig.input.bind(ig.KEY.Q, 'leftTrigger');
				ig.input.bind(ig.KEY.E, 'rightTrigger');
			},
			unbindTriggers: function() {
				ig.input.unbind(ig.KEY.Q, 'leftTrigger');
				ig.input.unbind(ig.KEY.E, 'rightTrigger');
			},
			update: function() {
				this.parent();
