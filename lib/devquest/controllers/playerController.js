...
.defines(function() {
	...
	devquest.controllers.PlayerController = ig.Class.extend({
		...
		inventory: [], //inventory is shared by party
		...
		init: function(game, players) {
			...
			this.inventory.push(new devquest.consumables.HealthMod(0, 0, {
				friendlyName: 'Nips',
			...
			this.inventory.push(new devquest.consumables.HealthMod(0, 0, {
				friendlyName: 'Double Ristretto Venti Nonfat Organic Chocolate Brownie Frappuccino Extra Hot with Foam and Whipped Cream Upside Down Double Blended',
		...
		update: function() {
			...
			if (ig.input.pressed('rot-lft-2')) {
				console.log('playerController: rot-lft-2');
				this.switchPlayer(1);
			} 
			else if (ig.input.pressed('rot-rt-2')) {
				console.log('playerController: rot-rt-2');
				this.switchPlayer(-1);
			}
		},
