...
.defines(function() {
	...
	ig.TitleScreen = ig.GameExtended.extend({		
		...
		cfg: ig.CONFIG,
		options: ['00    //developer quest','10    var game = new Game();','20    game.load();','30    var options = game.config();'],
		selectedOpt: 1,
		menuTop: 100,
		textHeight: 15,
		textSpacing: 2,
		...
		run: function() {	
			...
			if (ig.input.pressed('up')) {
				this.selectedOpt = (this.selectedOpt--) == 1 ? this.options.length - 1 : this.selectedOpt;
			}
			...
			for (var i = 0; i < this.options.length; i++) {
				this.font.draw(this.options[i], 25, this.menuTop + i * this.textHeight, ig.Font.ALIGN.LEFT);
			}
