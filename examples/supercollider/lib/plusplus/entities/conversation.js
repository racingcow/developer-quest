ig.module(
    ...
    .defines(function () {
        ...
        ig.EntityConversation = ig.global.EntityConversation = ig.EntityTrigger.extend(/**@lends ig.EntityConversation.prototype */{
             ...
             *      settings: {...},
             *      // array of step property names (or single property name) that represents response choices.
             *      // this overrides 'order' when present, and next step is set to the selected choice.
             *      // if used, messageContainerEntity should be set to ig.UITextBubbleChoice, 
             *      // or a custom container that can display multiple responses.
             *      next: ['2','3'],
             *      // list of step orders that must first be visited before this step becomes a valid choice
             *      // can be array of step indicies or single step index (int)
             *      requiredSteps: ['4','5']
             * };
             * // the following is the same as the above
             * // only it adds onto the end of the conversation
             * conversation.addStep( 'player', 'hello world', 2, {...}, ['2','3'], ['4','5'] );
             */
            steps: {},
             ...
             * @param {Array|String} [next] order(s) of steps that should be possible choices
             * @param {Array|String} [requiredSteps] order(s) of steps that must first be visited before this step is displayed as a possible choice
             */
            addStep: function ( text, speakerName, order, settings, next, requiredSteps ) {
            ...
            nextStep: function (next) {
                ...
                if ( this.stepsUnused.length > 0 ) {
                    ...
                    if (next) {
                        _ut.arrayCautiousRemove(this.stepsUnused, this.steps[next]);
                        this._step = this.steps[next];
                    }
                    else {
                        this._step = this.stepsUnused.shift();
                    }
                    _ut.arrayCautiousAdd( this.stepsUsed, this._step );

                    //todo: for debugging only - remove
                    for (var pname in this.steps) {
                        if (this.steps[pname] === this._step) {
                            break;
                        }
                    }

                    // parse the choices for this step
                    var names = !this._step.next ? [] : this._step.next instanceof Array ? this._step.next : this._step.next.toString().split(','), 
                        name, step, reqs, req, reqsVisited;
                    this._choices = [];
                    for ( var i = 0; i < names.length; i++ ) {
                        ...
                        // all required steps must be already visited for step to be a valid choice
                        reqs = !step.requiredSteps ? [] : step.requiredSteps instanceof Array ? step.requiredSteps : step.requiredSteps.toString().split(',');
                        reqsVisited = true;
                        for (var j = 0; j < reqs.length; j++) {
                            req = reqs[j];
                            reqsVisited &= (ig.utils.indexOfValue(this.stepsUsed, this.steps[req]) > 0);
                        }

                        if (reqsVisited) this._choices.push( { text: step.text, id: name } );
                    }
            ...
            showStep: function () {
                ...
                if ( this._step && this._step.text ) {
                    ...
                    if ( this.messageContainerEntity ) {
                        ...
                        //draw containers with multiple choices differently
                        // if (this._choices.length > 1) {
                        //     ig.merge(messageSettings, {
                        //         //sizeAsPct: false,
                        //         // shrinkToText: false,
                        //         textSettings: {
                        ...
                        messageSettings.textSettings = messageSettings.textSettings || {}; 
                        messageSettings.textSettings.text = this._step.text;
                        ...
                        if (this._choices.length > 1) {
                            messageContainer.onChoiceActivated.add( this.choiceActivated, this );
                        }
            ...
            activateStep: function () {
                ...
                if ( this._step ) {
                    ...
                    if ( message ) {
                        ...
                        //  steps with choices can't auto advance

                        if ( this.autoAdvance && !this.step.next) {
            ...
            hideStep: function () {
                ...
                if ( this.messageContainer ) {
                    ...
                    if (this._choices.length > 1) {
                        this.messageContainer.onChoiceActivated.remove( this.choiceActivated, this );
                    }
            ...
            update: function () {
                ...
                if ( this.step ) {
                    ...
                    // auto advance
                    else if ( ( ( this.autoAdvance && !this.step.next ) || ( this.step && !this.step.text ) ) && this.advanceTimer.delta() >= 0 ) {

                        this.nextStep();

                    }
                    // wait for action. steps with multiple choices will wait for messageContainer to call nextStep.
                    else if ( this._choices.length < 2 && ( ig.input.pressed( this.action ) || ig.input.released( this.action ) ) ) {

                        this.nextStep(this._choices[0] && this._choices[0].id);
            ...
            choiceActivated: function(next) {

                var step = this.steps[next];

                _ut.arrayCautiousRemove( this.stepsUnused, step );
                _ut.arrayCautiousAdd( this.stepsUsed, step );
